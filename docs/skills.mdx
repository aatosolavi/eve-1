...
`read_skill_file` progressively loads nested Markdown references from any loaded skill by skill name and relative path. Static skills do not require a sandbox: `load_skill` returns their instructions directly from the compiled agent, and `read_skill_file` reads their nested Markdown from compiled data. Dynamic skills use their runtime sandbox package for both operations. Access to non-Markdown package files also requires a sandbox. Builds may prewarm sandbox templates that contain static package files. Sandbox-backed files are available under `$HOME/.agents/skills/<skill>/`, with `/workspace/skills/<skill>/` as the fallback when `$HOME` is unavailable. Sibling references inside `SKILL.md`, such as `references/checklist.md`, are relative to the directory containing that specific `SKILL.md`.
