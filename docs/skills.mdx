...
`load_skill` returns static skill instructions directly from the compiled agent, so declaring or loading a static skill does not open a sandbox. Supporting package files are placed under `$HOME/.agents/skills/<skill>/` when sandbox-backed file access is needed; if `$HOME` is unavailable there, eve falls back to `/workspace/skills/<skill>/`. Sibling references inside `SKILL.md`, such as `references/checklist.md`, are relative to the directory containing that specific `SKILL.md`. Runtime-generated dynamic skills remain sandbox-backed.
