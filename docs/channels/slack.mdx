...
For a more selective policy, unsubscribe when someone hands the thread to another person. The fixed acknowledgement is posted directly to Slack and does not invoke the agent:
...
`ctx.unsubscribe()` silences automatic replies while preserving the session and its history. Explicitly mentioning the bot clears the marker and resumes the same session. Marker updates are processed by the session workflow, so an unsubscribe may take effect after an in-flight turn finishes. Mention detection is only a routing signal: adapt the condition if references such as “summarize what @Alex said” should not count as a handoff.
