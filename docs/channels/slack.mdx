...
- `onEvent(ctx, event)` observes every Events API callback, including types with no dedicated hook (`reaction_added`, `channel_created`, …). Subscribe the Slack app to each event type you want to see. Mentions and DMs reach it too, alongside their dedicated hooks; it never dispatches a turn — use `ctx.slack.request(...)` (or `ctx.thread` when the event carries a channel) for side effects.
