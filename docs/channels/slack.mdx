...
`ctx.unsubscribe()` silences automatic replies while preserving the session and its history. Explicitly mentioning the bot clears the marker and resumes the same session.
