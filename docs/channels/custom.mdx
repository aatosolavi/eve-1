...
- `send(message, { auth, continuationToken, state?, title?, turnPolicy? })` starts or resumes a session. `title` overrides the new workflow session's display title without changing the model message. `turnPolicy` controls input admitted while work is active. Returns a `Session`.
- `cancelTurn({ continuationToken, turnId? })` stops active work without supplying replacement input. The token uses the same channel-local format as `send()`.
...
`send()` defaults to `turnPolicy: "queue"`. If a turn is active, eve durably holds the input until that turn settles and then starts the next turn. Use this for ordered work where every response should finish. Input sent after the agent explicitly requests a response can satisfy that pending request; input queued earlier cannot accidentally answer a prompt raised later.

Use `turnPolicy: "steer"` when a new message should change the active turn's direction:
...
POST("/message", async (req, { send }) => {
  ...
  const session = await send(body.message, {
    auth: null,
    continuationToken: body.threadId,
    turnPolicy: "steer",
  });
...
Steering is an input policy, not a second kind of cancellation. eve applies the message at the next safe step boundary and keeps the same logical turn. A model or tool call already in progress may reach that boundary first, and side effects that already happened are not rolled back. If the turn wins the completion race, eve preserves the message as input for the next turn.

Use `cancelTurn()` for a stop action that has no replacement message:

```ts
POST("/threads/:threadId/stop", async (_req, { cancelTurn, params }) => {
  const result = await cancelTurn({ continuationToken: params.threadId });
  return Response.json(result);
});
```

The result is `{ status: "cancelling" }` when the active turn accepted the request or `{ status: "no_active_turn" }` when the session is unknown, idle, parked, or already finished. An optional `turnId` guard prevents a delayed stop request from affecting a newer turn. Observe `turn.cancelled` on the event stream to confirm settlement.
...
`WS()` handlers receive the same helpers as HTTP route handlers: `send`, `cancelTurn`, `getSession`, `receive`, `params`, `waitUntil`, and `requestIp`. The returned hooks are eve-owned structural types compatible with Nitro/H3 websocket routing, including `upgrade`, `open`, `message`, `close`, and `error`.
...
Each call to `send(message, { auth, continuationToken, state?, title?, turnPolicy? })` from a channel route addresses a session by its channel-local raw token. The framework prepends the channel name, derived from the file stem under `agent/channels/`, before handing the token to the runtime.
