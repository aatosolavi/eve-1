...
export default defineEvalConfig({
  reporters: [Datadog({ mlApp: "weather-agent" })],
});
```

Install the Datadog tracer and enable LLM Observability before running the eval. `mlApp` overrides `DD_LLMOBS_ML_APP`; omit it to use the environment default:
...
DD_LLMOBS_ENABLED=1 \
DD_LLMOBS_ML_APP=weather-agent \
