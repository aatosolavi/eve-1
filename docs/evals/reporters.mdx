...
Install the Datadog tracer before running the eval. The reporter initializes LLM Observability agentlessly, which is the usual CI setup. `projectName` is the Datadog LLM Observability project (ML application); it overrides `DD_LLMOBS_ML_APP` and falls back to `DD_SERVICE`.
...
Pass CI secrets through `apiKey` and `appKey` as shown, or leave them out of the config to use `DD_API_KEY` and `DD_APP_KEY`. Agentless LLM Observability ingestion authenticates with the API key; the application key is accepted for other Datadog APIs in the same process but is not required to send eval results. Set `agentless: false` only when a Datadog Agent is available.
