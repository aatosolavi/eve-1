...
| `workflowBridge`     | `readonly string[]`   | unset                  | Next-owned Workflow export names that eve tools may start through the World selected by `withWorkflow()`.                                                  |
...
## Start Next-owned workflows from eve tools

When the app also uses the Workflow SDK, compose `withWorkflow()` inside `withEve()` and allowlist
the workflow export names that eve may start:

```ts title="next.config.ts"
import { withEve } from "eve/next";
import { withWorkflow } from "workflow/next";

export default withEve(withWorkflow({}), {
  workflowBridge: ["processOrder"],
});
```

Then start the workflow from an authored tool without importing its transformed function:

```ts title="agent/tools/process-order.ts"
import { startNextWorkflow } from "eve/next/workflows";
...
export default defineTool({
  ...
  async execute({ orderId }) {
    return await startNextWorkflow("processOrder", [orderId]);
  },
});
```

`withWorkflow()` emits the transformed workflow manifest and selects the application's Workflow
World. `withEve()` validates the allowlist against that manifest and embeds a separate target World
client in the eve service. The tool dispatches `{ workflowId, args }` directly through that World;
it does not register the application workflow in eve and does not call a generated HTTP bridge.
World authentication and deployment targeting therefore follow the selected Workflow World: local
development uses the Next app's local World, Vercel uses ambient Vercel credentials, and a custom
self-hosted World reads its normal runtime configuration.

The returned promise resolves with `{ runId, workflow }` after the target World accepts the run. It
does not wait for workflow completion. Export names must be unique across the Workflow manifest;
`withEve()` fails the build when a requested name is missing or ambiguous.
