...
For cross-origin cookie authentication, set `credentials: "include"`; the server's CORS policy must also allow credentials and the calling origin. The option applies to every request and stream reconnect, while a raw `client.fetch()` call can override it for that request:
