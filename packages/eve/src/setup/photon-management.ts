...
async function registerUser(
  ...
  const user = record(body["user"] ?? body["data"] ?? body);
  return typeof user["assignedPhoneNumber"] === "string" ? user["assignedPhoneNumber"] : undefined;
}
...
export async function provisionPhotonProject(
  ...
  if (!E164.test(options.phoneNumber)) {
    throw new Error("Photon phone number must use E.164 format, for example +15551234567.");
  }
...
/** Registers the Connect ingress URL and returns its one-time signing secret. */
