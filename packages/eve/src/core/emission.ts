import type { HarnessEmissionState } from "#core/emission-state.js";
import type {
  ModelMessage,
  TextStreamPart,
  ToolSet,
...
import type { AssistantStepFinishReason, RuntimeIdentity } from "#core/protocol/message.js";
import {
...
interface StreamActionEmissionOptions {
  ...
  /**
   * Detects an inline tool result that represents a pending authorization
   * interrupt. Callers with an ambient context supply the stash-aware
   * predicate; the default covers the context-free model-output shape.
   */
  readonly isAuthorizationToolResult?: (result: TypedToolResult<ToolSet>) => boolean;
...
async function consumeStreamContent(
  ...
  const isAuthorizationToolResult =
    options?.isAuthorizationToolResult ??
    ((result: TypedToolResult<ToolSet>) => isPendingAuthorizationToolOutput(result.output));
    ...
    switch (part.type) {
      ...
      case "tool-result": {
        ...
        if (toolCallIdsSeenInStream.has(part.toolCallId)) {
          if (isAuthorizationToolResult(inlineToolResult)) {
            break;
          }
