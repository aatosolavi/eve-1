...
export async function emitStepActions(
  ...
  for (const result of reconcileToolResults(step)) {
    ...
    if (
      shouldSkipAuthorizationActionResult(
        result.callId,
        rawOutput,
        options.readStashedToolInterrupt,
      )
    ) {
      continue;
    }
...
function shouldSkipAuthorizationActionResult(
  callId: string,
  rawOutput: unknown,
  ...
  const stashed = readStashedToolInterrupt(callId);
