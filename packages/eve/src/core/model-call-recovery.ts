...
export function buildModelCallFailureDetails(input: {
  ...
  readonly formatError: (error: unknown, errorId: string) => JsonObject;
  ...
  return { ...input.formatError(error, errorId), ...modelCallDetails };
}
...
export async function attemptUnsupportedProviderToolRecovery(input: {
  ...
  input.log.warn("disabling unsupported provider tool(s); retrying step once", {
...
export async function attemptEmptyResponseRecovery(input: {
  ...
  input.log.warn("empty model response; reissuing the model call once", {
    sessionId: input.sessionId,
    turnId: input.turnId,
  });
