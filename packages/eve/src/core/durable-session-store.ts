...
/**
 * Reads the legacy `eve.session` stream tail for states persisted before
 * snapshot-carrying step results. Supplied by the engine that owns the
 * stream (the Workflow runtime); absent in engines whose states always
 * carry snapshots.
 */
export type LegacySessionTailReader = (sessionId: string) => ReadableStream<unknown>;
 ...
 */
export async function readDurableSession(
  state: DurableSessionState,
  options?: { readonly readLegacyTail?: LegacySessionTailReader },
): Promise<DurableSession> {
  if (state.snapshot !== undefined) {
    return migrateDurableSessionSnapshot(state.snapshot).session;
  }

  if (options?.readLegacyTail === undefined) {
    throw new Error(
      `Durable session state for run ${state.sessionId} carries no snapshot and no legacy tail reader was provided.`,
    );
  }
  const stream = options.readLegacyTail(state.sessionId);
