...
import { COMPACTION_PROMPT_ENVELOPE } from "#core/compaction-prompt.js";
import {
  compactMessages,
  getInputTokenCount,
  resolveCompactionModel,
  shouldCompact,
  type CompactionTextGenerator,
} from "#core/compaction.js";
import { estimateTokens } from "#core/token-estimate.js";
import type { CompactionConfig } from "#core/step-types.js";
...
): Promise<{ result: ModelMessage[]; summarizer: ReturnType<typeof vi.fn> }> {
  const summarizer = vi
    .fn<CompactionTextGenerator>()
    .mockResolvedValue(overrides.summary ?? "checkpoint text");
...
describe("compactMessages: summarization fallback", () => {
  ...
  it("forwards model options to the summarization call", async () => {
    const generateSummary = vi.fn<CompactionTextGenerator>().mockResolvedValue("summary");
