...
export async function dispatchRuntimeActionsStep(input: {
  ...
  const plan = planDispatch({
    ...
    bundle,
    ctx,
    parentNodeId: bundle.nodeId ?? ROOT_RUNTIME_AGENT_NODE_ID,
...
function planDispatch(input: {
  ...
  return input.actions.map((action): DispatchPlanEntry => {
    ...
    return classifyFreshStart({
      ...
      protectedDynamicSkillNames: input.protectedDynamicSkillNames,
...
function classifyFreshStart(input: {
  ...
  switch (action.kind) {
    case "subagent-call": {
      ...
      const description =
        dynamicAgentConfig?.description ?? localDefinition?.description;
      ...
      if (description !== undefined) {
        const localSource: {
          ...
          >;
          inherit?: NonNullable<typeof localDefinition>["inherit"];
          parentNodeId?: string;
          type: "local";
        } = {
          description,
        ...
        if (localDefinition?.inherit !== undefined) {
          localSource.inherit = localDefinition.inherit;
        }
        if (effectiveSandbox !== undefined) {
          localSource.effectiveSandbox = effectiveSandbox;
        }
        if (willShareSandbox) {
          localSource.parentNodeId = input.parentNodeId;
        }
        source = localSource;
      } else {
        const runtimeSource: {
...
function createEffectiveSandboxSource(input: {
  ...
  const parentSandboxState = input.adapterState?.parentSandboxState as
    | RuntimeSession["sandboxState"]
    | undefined;
  const effectiveSandbox: {
    parentSandboxState?: RuntimeSession["sandboxState"];
