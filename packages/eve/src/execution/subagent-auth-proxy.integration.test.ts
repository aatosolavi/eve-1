...
function decodeEvent(chunk: Uint8Array): StampedHandleMessageStreamEvent {
  return JSON.parse(new TextDecoder().decode(chunk).trim()) as StampedHandleMessageStreamEvent;
}
