...
export function summarizeTurnEvents(events: readonly HandleMessageStreamEvent[]): TurnEventSummary {
  ...
  for (const event of events) {
    ...
    if (event.type === "authorization.required") {
      const pending: Mutable<PendingAuthorization> = {
        description: event.data.description,
        name: event.data.name,
      };
      if (event.data.authorization !== undefined) pending.authorization = event.data.authorization;
      if (event.data.webhookUrl !== undefined) pending.webhookUrl = event.data.webhookUrl;
      pendingAuthorizations.set(event.data.name, pending);
    }
