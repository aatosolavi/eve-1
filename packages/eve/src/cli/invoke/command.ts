...
export function registerInvokeCommand(input: {
  ...
  input.program
    ...
    .option("--wait", "Wait for completion or a blocking input/authorization event")
...
async function runInvokeCommand(input: {
  ...
  if (options.wait !== true) {
    throw new InvalidArgumentError(
      "Local eve invoke requires --wait because the development server owns the running invocation.",
    );
  }
...
async function executeWithSignals(
  ...
  try {
    const invokeInput: Mutable<RunInvokeInput> = {
      operation,
      signal: controller.signal,
      target,
      wait: input.options.wait === true,
    };
    if (headers !== undefined) invokeInput.headers = headers;
    const result = await input.deps.runInvoke(invokeInput);
...
function invokeExitCode(result: InvokeResult): number {
  if (result.status === "failed" || result.status === "authentication-required") return 1;
