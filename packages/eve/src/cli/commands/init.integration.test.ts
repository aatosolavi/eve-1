...
function dependencies(
  ...
  setTerminalDirectory: ReturnType<typeof vi.fn<InitCommandDependencies["setTerminalDirectory"]>>;
...
describe("runInitCommand", () => {
  it("creates the base agent with the runtime default model and invoking eve dependency", async () => {
    ...
    expect(output.messages[3]).toContain("npx skills add vercel/eve");
    expect(output.messages[4]).toContain("$ eve dev");
    expect(deps.setTerminalDirectory).toHaveBeenNthCalledWith(1, projectPath);
    expect(deps.setTerminalDirectory).toHaveBeenNthCalledWith(2, parentDirectory);
  });
