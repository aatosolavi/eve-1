...
export async function writeDevelopmentCompiledArtifactsFiles(input: {
  ...
  /**
   * The user's real project root (not the ephemeral dev-runtime snapshot).
   * Durable dev output — like `.eve/traces` — must live here so it survives
   * snapshot rotation and matches where `eve trace` reads.
   */
  ...
  if (materializedIndex.instrumentation !== undefined) {
    ...
    await writeFile(
      ...
      createInstrumentationPluginSource({
        agentName: input.compileResult.manifest.config.name,
        localDevTracing: {
          appRoot: input.projectAppRoot,
          registerPath: resolvePackageSourceFilePath("src/harness/local-dev-tracing.ts"),
        },
    ...
    // No authored instrumentation: enable zero-config local trace capture so
    // `eve dev` gets the trace waterfall with no setup. Dev-only by
    // construction — production (`writeCompiledArtifactsFiles`) never emits this.
    await writeFile(
      instrumentationPluginPath,
      createDevTracingPluginSource({
        agentName: input.compileResult.manifest.config.name,
        appRoot: input.projectAppRoot,
        registerPath: resolvePackageSourceFilePath("src/harness/local-dev-tracing.ts"),
      }),
...
function createDevTracingPluginSource(input: {
  agentName: string;
  appRoot: string;
  registerPath: string;
}): string {
  return [
    ...
    `import { registerLocalDevTracing } from ${stringifyEsmImportSpecifier(input.registerPath)};`,
    "",
    `registerLocalDevTracing({ appRoot: ${JSON.stringify(input.appRoot)}, agentName: ${JSON.stringify(input.agentName)} });`,
    "",
    ...
    "export default function installDevTracingPlugin() {}",
    "",
  ].join("\n");
}
...
function createInstrumentationPluginSource(input: {
  ...
  const localDevImport =
    input.localDevTracing === undefined
      ? []
      : [
          `import { registerLocalDevTracing } from ${stringifyEsmImportSpecifier(input.localDevTracing.registerPath)};`,
        ];
  const localDevRegistration =
    input.localDevTracing === undefined
      ? []
      : [
          `registerLocalDevTracing({ appRoot: ${JSON.stringify(input.localDevTracing.appRoot)}, agentName: ${JSON.stringify(input.agentName)} });`,
          "",
        ];
  return [
    "// Generated by eve. Do not edit by hand.",
