...
import {
  readDevelopmentRuntimeArtifactsWorker,
  type DevelopmentRuntimeArtifactsWorker,
...
export interface ActivatedDevelopmentRuntimeArtifactsGeneration {
  ...
  readonly runtimeAppRoot: string;
  readonly snapshotRoot: string;
  readonly worker: DevelopmentRuntimeArtifactsWorker;
}
  ...
  const generations = await Promise.all(
    entries
      ...
      .map(async (entry): Promise<ActivatedDevelopmentRuntimeArtifactsGeneration | undefined> => {
        ...
        const worker = await readDevelopmentRuntimeArtifactsWorker(snapshotRoot);
        if (worker === undefined) {
          return undefined;
        }
        return {
