...
const WORKFLOW_FLOW_ROUTE = "/.well-known/workflow/v1/flow";
const WORKFLOW_STEP_ROUTE = "/.well-known/workflow/v1/step";
...
class LocalParentDevelopmentWorkflowWorld implements ParentDevelopmentWorkflowWorld {
  ...
  constructor(input: {
    ...
    this.#hasGeneration = input.hasGeneration;
  ...
  async handleRequest(request: Request): Promise<Response | undefined> {
    ...
    if (
      request.method === "POST" &&
      (url.pathname === WORKFLOW_FLOW_ROUTE || url.pathname === WORKFLOW_STEP_ROUTE)
    ) {
      return await this.#handleDelivery(request);
    }
    return undefined;
  }
  ...
  async #handleDelivery(request: Request): Promise<Response> {
    if (!this.#isTrusted(request)) {
      return Response.json({ error: "Workflow delivery is not trusted." }, { status: 401 });
    }
    try {
      const payload = decodeDevelopmentWorldJson(await request.clone().text());
      const generationId = await this.#resolveDeliveryGeneration(payload);
      this.#assertGenerationExists(generationId);
      return await this.#dispatch(request, generationId);
    } catch (error) {
      return Response.json({ error: String(error) }, { status: 500 });
    }
  }

  async #resolveDeliveryGeneration(payload: unknown): Promise<string> {
    if (!isObject(payload) || payload.__healthCheck === true) {
      return this.#resolveActiveGenerationId();
    }
    const initial = isObject(payload.runInput) ? payload.runInput : undefined;
    if (initial !== undefined) {
      return initial.workflowName === turnWorkflowReference.workflowId &&
        typeof initial.deploymentId === "string"
    ...
    const runId =
      typeof payload.runId === "string"
        ...
        : typeof payload.workflowRunId === "string"
          ...
          : undefined;
    if (runId === undefined) {
      throw new Error("Workflow delivery does not identify a run.");
    }
  ...
  async #queue(...args: Parameters<World["queue"]>): ReturnType<World["queue"]> {
    ...
    return await this.#world.queue(queueName, message, {
      ...
      headers: {
        ...options?.headers,
        [DEVELOPMENT_WORKFLOW_TRANSPORT_HEADER]: this.#transportSecret,
      },
    });
  }
  ...
  #assertGenerationExists(generationId: string): void {
    if (!this.#hasGeneration(generationId)) {
