...
import { resolveSessionLogPath } from "#internal/session-logs/files.js";
...
describe("development session log recorder", () => {
  it("restarts a failed stream follower on the next durable write", async () => {
    const appRoot = await createScratchDirectory("eve-session-log-follower-");
    ...
    const recorder = new DevelopmentSessionLogRecorder({ appRoot, world });
    try {
      ...
      await vi.waitFor(async () => {
        const source = await readFile(resolveSessionLogPath(appRoot, runId), "utf8");
