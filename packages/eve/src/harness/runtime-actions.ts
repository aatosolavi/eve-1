...
function applyInheritedSandboxResults(input: {
  ...
  for (const result of input.results) {
    if (
      ...
      !("inheritedSandbox" in result) ||
      result.inheritedSandbox === undefined
    ) {
      continue;
    }
...
function toPublicRuntimeActionResult(result: RuntimeActionResult): RuntimeActionResult {
  if (result.kind !== "subagent-result" || !("inheritedSandbox" in result) || result.inheritedSandbox === undefined) {
    return result;
  }

  // Strip eve-internal sandbox backfill from public stream projections.
  const { inheritedSandbox: _inheritedSandbox, ...publicResult } = result;
  return publicResult;
}
